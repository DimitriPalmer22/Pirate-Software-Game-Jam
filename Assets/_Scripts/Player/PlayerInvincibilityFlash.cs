...
public class PlayerInvincibilityFlash : ComponentScript<Player>
{
    ...
    private void Start()
    {
        ...
        ParentComponent.OnDamaged += GetRenderersOnDamaged;
        ParentComponent.OnDamaged += StartFlashingOnDamaged;
    }
    ...
    {
        // Call the stop flashing method if the player is no longer invincible
        if (_isFlashing && !ParentComponent.IsInvincibleBecauseDamaged)
            StopFlashing();
